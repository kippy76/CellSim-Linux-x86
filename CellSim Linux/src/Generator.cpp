...
#include "Generator.h"
#include <stdlib.h>
#include "SDL/SDL_video.h"	// 2D graphics library
#include <vector>
#include "Colours.h"
#include "Utils.h"
#include <cmath>
...
bool Generator::continueSimulationCheck()
{
	...
	for (unsigned int colonyIndex = 0; colonyIndex < model->getAllColonies()->size(); colonyIndex++)
	{
		// Immediate stop if either colony dead :
		colSize = ((*model->getAllColonies())[colonyIndex])->getColony()->size();
		if (colSize == 0)
		{
			return false;
		}
	...
	return cellCount > maxCells ? false : true;
}
	...
	for (int radius = 1; radius < modeY - (border * 2); radius++)
	{
		for (int currentX = x - radius; currentX <= x + radius; currentX++)
		{
			for (int currentY = y - radius; currentY <= y + radius; currentY++)
			{
				...
				{
					if ((getPixelColour(currentX, currentY) == Colours::black) || ((currentX == x)
							&& (currentY == y)))
					{
		...
		if (pointsFree.size() != 0)
		{
			...
			int chosen = Utils::getRandomInt(1, pointsFree.size()) - 1;
...
void Generator::physicsMove(Cell* cell)
{
	...
	float x = cell->getX();
	float y = cell->getY();
	Coords dest = getNearestFreePixel(x, y, true);
	if (isCoordsInBounds(x, y, false))
	{
		x -= ((int) x - dest.x);
		y -= ((int) y - dest.y);
	}
...
void Generator::physicsDivide(Cell* cell)
{
	...
	if (cell->mytosisPoll())
	{
		for (unsigned int loop = 0; loop < model->getAllColonies()->size(); loop++)
		{
			...
			{
				Coords dest = getNearestFreePixel(cell->getX(), cell->getY(), true);
				(*model->getAllColonies())[loop]->addNewCell(dest.x, dest.y, true, cell->getColour(),
...
{
	if (!isCoordsInBounds(cell->getX(), cell->getY(), false))
	{
		return;
	}
	float y = cell->getY();
	...
	float denominator = (modeY / 2) - border - wallWidth;
